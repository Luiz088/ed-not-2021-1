/*
    ...
    a clase empresta algumas características aos objetos que serão criados a partir
    dela, embora outras caracteríscitas dos objetos possam variar.
*/
...
class FormaGeometrica {
    ...
    /* Um método especial é o constructor(). Ele é chamado toda vez que se vai criar
     ...
     constructor(base, altura, tipo) {
        ...
        // Invocar os setter das propriedades. As validações serão
        // feitas por lá
        this.base = base
        this.altura = altura
        this.tipo = tipo
        ...
        // escopo do objeto criando a partir da classe. Essas variáveis especiais são 
        // denominadas ATRIBUTOS e diferenciam-se das variáveis comuns pela presença do prefixo this.
     ...
     /*
        getters: são métodos especiais que permitem expor o vetor dos atributos privados
    ...
    */
   set base(valor) {
       // A base deve ser numérica e seu valor maior que zero
        if(isNaN(valor) || valor <= 0) throw new Error('A base deve ser numérica e seu valor maior que zero')

        this.#base = valor  // O atributo #base recebe o novo valor validado
   }

   set altura(valor) {
       // A altura deve ser numérica e seu valor maior que zero
        if(isNaN(valor) || valor <= 0) throw new Error('A altura deve ser numérica e seu valor maior que zero')
   ...
   /*
        ...
        passamos a nos regerir a ele como PROPRIEDADE do objeto.
   */

   /************************************************************************ */

   // Funções, dentro de classes, passam a ser denomidadas MÉTODOS e perdem
   // a palavra funtion
   calcularArea() {
       switch(this.tipo) {
            case 'Q':
...
catch(erro) { // O bloco cathc() recebe o erro que foi gerado no bloco try
...
//forma1.base = 'shdfsaas'    // Não funcionar
